...
#define RESULT_DECLARE(type)                                                   \
  ...
  {                                                                            \
    ...
    const char* src_function;                                                  \
                                                                               ...
                                                                               \
  Result(type) ___RESULT_##type##_declare(const ErrorType* error,              \
                                          int src_line,                        \
                                          char* src_file,                      \
                                          const char* src_function,            \
                                          type value);                         \
                                                                               \
  type ___RESULT_##type##_unwrap(int src_line,                                 \
                                 char* src_file,                               \
                                 const char* src_function,                     \
                                 Result(type) self);                           \
                                                                               \
  type ___RESULT_##type##_unwrap_or(Result(type) self, type fallback);         \
                                                                               \
  const ErrorType* ___RESULT_##type##_unwrap_err_or(                           \
    Result(type) self, const ErrorType* fallback);                             \
                                                                               \
  type ___RESULT_##type##_expect(int src_line,                                 \
                                 char* src_file,                               \
                                 const char* src_function,                     \
                                 Result(type) self,                            \
                                 const char* error);                           \
                                                                               \
  const ErrorType* ___RESULT_##type##_expect_err(int src_line,                 \
                                                 char* src_file,               \
                                                 const char* src_function,     \
                                                 Result(type) self,            \
                                                 const char* error);           \
                                                                               \
  const ErrorType* ___RESULT_##type##_unwrap_err(int src_line,                 \
                                                 char* src_file,               \
                                                 const char* src_function,     \
                                                 Result(type) self);           \
                                                                               \
  Result(type) ___RESULT_##type##_and(Result(type) self, Result(type) other);  \
                                                                               \
  Result(type)                                                                 \
    ___RESULT_##type##_and_then(Result(type) self, Result(type) (*c)(type));   \
                                                                               \
  ...
  const ErrorType* ___RESULT_##type##_unwrap_err_or(                           \
    Result(type) self, const ErrorType* fallback);                             \
                                                                               \
  Result(type) ___RESULT_##type##_or_else(                                     \
    Result(type) self, Result(type) (*c)(const ErrorType*));                   \
                                                                               \
...
#define RESULT_DEFINE(type)                                                    \
  ...
  {                                                                            \
    ...
    return result;                                                             \
                                                                               ...
                                                                               \
  type ___RESULT_##type##_unwrap(                                              \
    int src_line, char* src_file, const char* src_function, Result(type) self) \
  {                                                                            \
    if (is_err(self))                                                          \
  ...
  {                                                                            \
    ...
    return self.error;                                                         \
                                                                               ...
                                                                               \
  const ErrorType* ___RESULT_##type##_unwrap_err(                              \
    int src_line, char* src_file, const char* src_function, Result(type) self) \
  {                                                                            \
    if (is_ok(self)) {                                                         \
      panic_function(src_line,                                                 \
...
#define result_and(type, self, other) ___RESULT_##type##_and(self, other)
...
#define result_or_else(type, self, call) ___RESULT_##type##_or_else(self, call)
...
#define result_OK(...)                                                         \
  ___RESULT_OK_HELPER(___RESULT_HAS_COMMA(__VA_ARGS__))                        \
  (__VA_ARGS__)
...
Result(void)
  ___RESULT_void_and_then(Result(void) self, Result(void) (*c)(void));
