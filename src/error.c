...
static inline void ___Error_obtain_backtrace(Error* error)
{
/* TODO: Win32 implementation */
    ...
    for (;;) {
        ...
        error->backtrace_size = backtrace(error->backtrace, allocated_backtrace_size);
        ...
        error->backtrace_symbols = backtrace_symbols(error->backtrace, error->backtrace_size);
        break;
    }
#else
    error->backtrace = NULL;
...
Error* ___Error_construct(const ErrorType* type, char* custom_message, Error* source, char* src_file, int src_line, const char* src_function)
{
...
{
    if (error->custom_message) free(error->custom_message);
    if (error->source) free(error->source);
    if (error->backtrace) {
        free(error->backtrace);
